...
//			console.log("updateTotalProgress")
//			var loaded = 0,
//				total = 0,
...
//			percentages[file.id][1] = Math.round(percentage);
//			
//			updateTotalProgress();
//		};
//		//显示上传的框框
...
$(document).on("click",".img",function(){
	...
	delImagePreviews($(this).attr("index"));
	dd.innerHTML="";imgcount--;
	for(var i = 0; i < imgbase.length; i++) {
			dd.innerHTML += "<div> <img src='" + imgbase[i] + "' index='" + i + "' class='img' width='100px' height='100px'/> </div>";
	}
...
function setImagePreviews(avalue){
	...
	for(var i = 0; i < fileList.length; i++) {
		...
		fsrc = window.URL.createObjectURL(fileList[i]);
		convertImgToBase64(fsrc, function(base64Img) {
			imgbase.push(base64Img);	
		});
	}
